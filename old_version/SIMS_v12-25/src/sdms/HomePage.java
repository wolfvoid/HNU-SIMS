...
public class HomePage { //主页类
	static JFrame mainJFrame=new JFrame("学校信息管理系统");
	static Container con=mainJFrame.getContentPane();
	static int flag; //是否为管理员
	...
	public static void welcomePage() { //欢迎界面
		...
		pn.setSize(550,400);
		pn.setLayout(null);
		JLabel lb1=new JLabel("欢迎使用"),lb2=new JLabel("学校信息管理系统");
		JButton bt1=new JButton("学生登录"),bt2=new JButton("宿管登录"),bt3=new JButton("教师登录"),bt4=new JButton("管理员登录"),bt_changeAppearance=new JButton("更换外观",new ImageIcon("image/更换外观.png"));
		...
		bt2.setBounds(300,180,180,70);
		bt2.setCursor(new Cursor(Cursor.HAND_CURSOR));

		bt3.setFont(new Font("黑体",0,22));
		bt3.setBounds(50,270,180,70);
		bt3.setCursor(new Cursor(Cursor.HAND_CURSOR));

		bt4.setFont(new Font("黑体",0,22));
		bt4.setBounds(300,270,180,70);
		...
		bt2.addActionListener(new ActionListener() { //进入宿管登录界面
			public void actionPerformed(ActionEvent e) {
				flag=2;
				con.remove(pn);
				mainJFrame.repaint();
		...
		bt3.addActionListener(new ActionListener() { //进入老师登录界面
			public void actionPerformed(ActionEvent e) {
				flag=3;
				con.remove(pn);
				mainJFrame.repaint();
		...
		bt4.addActionListener(new ActionListener() { //进入管理员登录界面
			public void actionPerformed(ActionEvent e) {
				flag=4;
				con.remove(pn);
				mainJFrame.repaint();
		...
		bt2.addActionListener(new ActionListener() { //登录，连接数据库获取账号密码并检验，正确则进入功能界面
			public void actionPerformed(ActionEvent e) {
				...
				switch(flag)
				{
					...
					//宿管
					case 2:
						new Login().dor_adminLogin(tf.getText(),String.valueOf(pf.getPassword()));
						break;
					//教师
					case 3:
						new Login().teacherLogin(tf.getText(),String.valueOf(pf.getPassword()));
						break;
					//管理员
					case 4:
...
//					new Login().studentLogin(tf.getText(),String.valueOf(pf.getPassword()));;
	...
	public static void databaseConnection() { //连接MySQL数据库
		...
		final String DB_URL="jdbc:mysql://localhost:3306/db_project4?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=UTC"; //数据库URL
		final String USER="root"; //数据库用户名
		final String PASS="1024"; //数据库密码
		try {
