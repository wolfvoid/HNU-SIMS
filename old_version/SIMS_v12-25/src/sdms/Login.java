...
public class Login { //登录类
    ...
    public void dor_adminLogin(String account, String password) { //宿舍管理员登录
        boolean flag = false; //是否查找到相应账号密码
        String dor_admin_num = null, pass; //宿舍管理员账号密码
        try {
            ...
            ResultSet rs = ps.executeQuery(sql); //ResultSet类，用来存放获取的结果集
            while (rs.next()) { //遍历结果集
                dor_admin_num = rs.getString("admin_num");
                pass = rs.getString("password");
                if (account.equals(dor_admin_num) && password.equals(pass)) {
                    flag = true;
                    break;
                }
            }
        ...
        if (flag) { //若宿舍管理员账号密码正确，则进入学生功能界面
            lb_tips.setText("登录成功！");
            loginTips();
            new DorAdminFunction(dor_admin_num);
            HomePage.mainJFrame.dispose();
        } else { //若账号密码错误，则进行提示
            lb_tips.setText("账号或密码错误！");
            loginTips();
    ...
    public void teacherLogin(String account, String password) { //老师登录
        boolean flag = false; //是否查找到相应账号密码
        String teacher_num = null, pass; //老师账号密码
        try {
            String sql = "SELECT tno, password FROM teacher"; //SQL语句
            PreparedStatement ps; //创建PreparedStatement类对象ps，用来执行SQL语句
            ps = HomePage.connection.prepareStatement(sql); //把操作数据库返回的结果保存到ps中
            ResultSet rs = ps.executeQuery(sql); //ResultSet类，用来存放获取的结果集
            while (rs.next()) { //遍历结果集
                teacher_num = rs.getString("tno");
                pass = rs.getString("password");
                if (account.equals(teacher_num) && password.equals(pass)) {
                    flag = true;
                    break;
                }
            }
        ...
        if (flag) { //若老师账号密码正确，则进入老师功能界面
            lb_tips.setText("登录成功！");
            loginTips();
            new TeacherFunction(teacher_num);
            HomePage.mainJFrame.dispose();
        } else { //若账号密码错误，则进行提示
            lb_tips.setText("账号或密码错误！");
            loginTips();
