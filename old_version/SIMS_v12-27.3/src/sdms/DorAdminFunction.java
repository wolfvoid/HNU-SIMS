    ...
    public void allInout() { //该宿管管理的宿舍楼的所有出入信息
        ...
        if (count != 0) {
            MyEvent e = new MyEvent() { //点击“删除”按钮，可删除出入信息
                public void invoke(ActionEvent e) {
                    MyButton button = (MyButton) e.getSource();
                    String sql = "DELETE FROM in_out WHERE stu_num='" + table.getValueAt(button.getRow(), button.getColumn() - 5) + "' AND floor_num='" + table.getValueAt(button.getRow(), button.getColumn() - 3) + "' AND category='" + table.getValueAt(button.getRow(), button.getColumn() - 2) + "' AND time='" + table.getValueAt(button.getRow(), button.getColumn() - 1) + "'";
        ...
        bt_confirm.addActionListener(new ActionListener() { //确认添加出入信息
            public void actionPerformed(ActionEvent e) {
                ...
                if (tf[0].getText().equals("") || tf[2].getText().equals("") || tf[3].getText().equals("") || cb_inorout.getSelectedIndex() == 0) {
                    ...
                    if (cb_inorout.getSelectedItem() == "          出")
                        inorout = "出";
                    else if (cb_inorout.getSelectedItem() == "          入")
                        inorout = "入";
                    String sql; //SQL语句
                    PreparedStatement ps; //创建PreparedStatement类对象ps，用来执行SQL语句
                    ResultSet rs; //ResultSet类，用来存放获取的结果集
    ...
    public void queryInoutInfo_2(String sql) { //查询出入信息结果
        ...
        if (count != 0) {
            MyEvent e = new MyEvent() { //点击“删除”按钮，可删除出入信息
                public void invoke(ActionEvent e) {
                    MyButton button = (MyButton) e.getSource();
                    String delete_sql = "DELETE FROM in_out WHERE stu_num='" + table.getValueAt(button.getRow(), button.getColumn() - 5) + "' AND floor_num='" + table.getValueAt(button.getRow(), button.getColumn() - 3) + "' AND category='" + table.getValueAt(button.getRow(), button.getColumn() - 2) + "' AND time='" + table.getValueAt(button.getRow(), button.getColumn() - 1) + "'";
                    lb_tips.setText("是否确定删除该出入信息？");
        ...
        if (count != 0) {
            MyEvent e = new MyEvent() { //点击“查看”按钮，查看信息，可修改和删除信息
                public void invoke(ActionEvent e) {
                    ...
                    String yesno;
                    if (table.getValueAt(row, button.getColumn() - 1).equals("<html><font color='green'>是</font></html>"))
                        yesno = "是";
                    else
                        yesno = "否";
                    pn_first_2.removeAll();
    ...
    public void visitRepairInfo(String stu_num, String floor_num, String layer, String room_num, String info, String yes_no, int x, String query_sql) { //查看宿舍报修信息，可删除
        ...
        try {
            ...
            while (rs.next()) {
                ...
                //未选中为否
                tbt_yesno.setSelected(repair.yes_no.equals("是")); //选中为是
            }
            rs.close();
            ps.close();
        } catch (SQLException e) {
