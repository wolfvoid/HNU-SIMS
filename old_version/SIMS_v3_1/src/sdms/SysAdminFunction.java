...
public class SysAdminFunction implements ActionListener { //系统管理员功能类
    JFrame sysAdminJFrame = new JFrame("学校信息管理系统-----[系统管理模式]");
    ...
    public SysAdminFunction() { //整体界面
        ...
        bt_student.setBounds(0, 50, 249, 50);

        JButton bt_teacher = new JButton("教师管理"); //[教师管理]
        bt_teacher.setFont(new Font("黑体", 0, 20));
        bt_teacher.setContentAreaFilled(false);
        bt_teacher.setBounds(0, 105, 249, 50);

        JButton bt_course = new JButton("课程管理"); //[课程管理]
        bt_course.setFont(new Font("黑体", 0, 20));
        bt_course.setContentAreaFilled(false);
        bt_course.setBounds(0, 160, 249, 50);

        JButton bt_select_course = new JButton("选课管理"); //[选课管理]
        bt_select_course.setFont(new Font("黑体", 0, 20));
        bt_select_course.setContentAreaFilled(false);
        bt_select_course.setBounds(0, 215, 249, 50);

        JButton bt_admin = new JButton("宿管管理");//[宿管管理]
        bt_admin.setFont(new Font("黑体", 0, 20));
        bt_admin.setContentAreaFilled(false);
        bt_admin.setBounds(0, 270, 249, 50);

        JButton bt_dormitory = new JButton("宿舍管理"); //[宿舍管理]
        bt_dormitory.setFont(new Font("黑体", 0, 20));
        bt_dormitory.setContentAreaFilled(false);
        bt_dormitory.setBounds(0, 325, 249, 50);

        JButton bt_floor = new JButton("宿舍楼管理"); //[宿舍楼管理]
        bt_floor.setFont(new Font("黑体", 0, 20));
        bt_floor.setContentAreaFilled(false);
        bt_floor.setBounds(0, 380, 249, 50);

        JButton bt_stay = new JButton("住宿管理"); //[住宿管理]
        bt_stay.setFont(new Font("黑体", 0, 20));
        bt_stay.setContentAreaFilled(false);
        bt_stay.setBounds(0, 435, 249, 50);
        ...
        bt_allocation.setBounds(0, 490, 249, 50);
        ...
        bt_teacher.addActionListener(this);
        bt_course.addActionListener(this);
        bt_select_course.addActionListener(this);
        ...
        pn_menu.add(bt_out);
        pn_menu.add(bt_teacher);
        pn_menu.add(bt_course);
        pn_menu.add(bt_select_course);
    ...
    public void actionPerformed(ActionEvent e) { //依据选择的菜单功能，进行相应功能
        if (e.getActionCommand().equals("退出")) {
            ...
            } else if (e.getActionCommand().equals("教师管理")) {
                lb_topFunction.setText("[教师管理]");
                pn_function.add(teacherManage());
            } else if (e.getActionCommand().equals("课程管理")) {
                lb_topFunction.setText("[课程管理]");
                pn_function.add(courseManage());
            }else if (e.getActionCommand().equals("选课管理")) {
                lb_topFunction.setText("[选课管理]");
                pn_function.add(select_course_Manage());
        ...
        bt_confirm.addActionListener(new ActionListener() { //确认添加宿舍楼信息
            public void actionPerformed(ActionEvent e) {
                ...
                if (tf[0].getText().equals("") || tf[1].getText().equals("") || tf[2].getText().equals("") || tf[3].getText().equals("") || tf[4].getText().equals("") || cb_sex.getSelectedIndex() == 0) {
                    ...
                    if (cb_sex.getSelectedItem() == "          男")
                        sex = "男";
                    else if (cb_sex.getSelectedItem() == "          女")
                        sex = "女";
                    else
                        sex = "混合";
    ...
    public void queryStudentInfo_2(String sql) { //查询学生信息结果
        String[] columnNames = {"工号", "姓名", "性别", "密码"}; //表格列名
        String[][] rowData = null; //表格数据
        ...
        try {
            ...
            if (count == 0) { //若无符合条件的学生
                ...
                int i = 0;
                do { //获取符合条件的学生信息
                    rowData[i][0] = rs.getString("tno"); //学号
                    rowData[i][1] = rs.getString("tname"); //姓名
                    rowData[i][2] = rs.getString("tsex"); //性别
                    rowData[i][3] = rs.getString("password") ; //年级
                    i++;
                } while (rs.next());
            }
            rs.close();
        ...
        JPanel pn_top = new JPanel();
        pn_top.setBounds(0, 0, 1045, 50);
        JLabel lb_num = new JLabel(" 共查询到" + count + "条教师信息！ ");
        lb_num.setFont(new Font("黑体", 0, 25));
        JButton bt_back = new JButton("返回", new ImageIcon("image/返回.png"));
        bt_back.setFont(new Font("黑体", 0, 17));
        bt_back.setContentAreaFilled(false);
    ...
    public void allTeacher() { //所有教师
        String[] columnNames = {"工号", "姓名", "性别", "操作"}; //表格列名
        String[][] rowData = null; //表格数据
        int count = 0; //表的元组总数
        try { //获取teacher表信息
            ...
            ResultSet rs = ps.executeQuery(sql); //ResultSet类，用来存放获取的结果集
            rs.last();
            count = rs.getRow(); //获取教师元组总数
            if (count == 0) { //若teacher表无元组
                rowData = new String[1][4];
                for (int i = 0; i < 4; i++)
                    rowData[0][i] = "无";
            } else { //若teacher表有元组
                ...
                int i = 0;
                do { //获取所有学生信息
                    rowData[i][0] = rs.getString("tno"); //教师号
                    rowData[i][1] = rs.getString("tname"); //姓名
                    rowData[i][2] = rs.getString("tsex"); //性别
                    i++;
                } while (rs.next());
        ...
        JPanel pn_top = new JPanel();
        pn_top.setBounds(0, 0, 1045, 50);
        JLabel lb_num = new JLabel("  共" + count + "条教师信息！  ");
        lb_num.setFont(new Font("黑体", 0, 25));
        JButton bt_add = new JButton("添加", new ImageIcon("image/add.png")), bt_export = new JButton("导出", new ImageIcon("image/导出.png"));
        bt_add.setBackground(Color.white.darker());
        bt_add.setFont(new Font("黑体", 0, 20));
        ...
        if (count != 0) {
            MyEvent e = new MyEvent() { //点击“查看”按钮，查看信息，可修改和删除信息
                public void invoke(ActionEvent e) {
                    MyButton button = (MyButton) e.getSource();
                    pn_first_2.removeAll();
                    visitTeacherInfo((String) table.getValueAt(button.getRow(), button.getColumn() - 3), 1, null);
                }
            };
        ...
        bt_add.addActionListener(new ActionListener() { //添加教师信息
            public void actionPerformed(ActionEvent e) {
                pn_first_2.removeAll();
                addTeacherInfo();
            }
    ...
    public void visitTeacherInfo(String tno_num, int x, String query_sql) { //查看教师信息，可修改和删除
        if (x == 1) {
            pn_first_2.removeAll();
            pn_first_2.setLayout(null);
            pn_first_2.setBounds(0, 0, 1045, 695);
        ...
        bt_back.setBorderPainted(false);
        bt_back.setCursor(new Cursor(Cursor.HAND_CURSOR));
        JButton bt_modify = new JButton("修改教师信息", new ImageIcon("image/modify.png")), bt_delete = new JButton("删除教师信息", new ImageIcon("image/delete.png")),
                bt_save = new JButton("保存", new ImageIcon("image/save.png")), bt_cancel = new JButton("取消", new ImageIcon("image/cancel.png"));
        bt_modify.setFont(new Font("黑体", 0, 17));
        bt_modify.setBounds(755, 380, 190, 40);
        bt_modify.setContentAreaFilled(false);
        ...
        try {
            String sql = "SELECT * FROM teacher WHERE tno='" + tno_num + "'"; //SQL语句
            PreparedStatement ps; //创建PreparedStatement类对象ps，用来执行SQL语句
            ps = HomePage.connection.prepareStatement(sql); //把操作数据库返回的结果保存到ps中
            ResultSet rs = ps.executeQuery(sql); //ResultSet类，用来存放获取的结果集
            while (rs.next()) {
                teacher = new Teacher(rs.getString("tno"), rs.getString("tname"), rs.getString("tsex"), rs.getString("password"));
                tf[0].setText(teacher.tno);
                tf[1].setText(teacher.tname);
                tf[2].setText(teacher.tsex);
                tf[3].setText(teacher.password);
            }
            rs.close();
            ps.close();
        } catch (SQLException e) {
        ...
        bt_modify.addActionListener(new ActionListener() { //修改教师信息
            public void actionPerformed(ActionEvent e) {
                for (int i = 0; i < 4; i++)
                    if (i != 3)
                        tf[i].setEditable(true);
        ...
        bt_delete.addActionListener(new ActionListener() { //删除教师信息
            public void actionPerformed(ActionEvent e) {
                lb_tips.setText("是否确定删除该教师信息？");
                if (x == 1)
                    choiceTips("");
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    if (tf[0].getText().equals("") || tf[1].getText().equals("") || tf[2].getText().equals("") || tf[3].getText().equals("")) {
                        ...
                        boolean flag = false; //代表工号是否重复
                        //检测学号是否重复
                        sql = "SELECT tno FROM teacher WHERE tno='" + tf[0].getText() + "'";
                        ps = HomePage.connection.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.TYPE_FORWARD_ONLY);
                        rs = ps.executeQuery(sql);
                        flag = rs.next();
                        rs.first();
                        flag = flag && !(tf[0].getText().equals(teacher.tno));
                        if (flag) { //学号重复，进行提示
                            lb_tips.setText("工号重复！");
                            functionTips();
                            flag = false;
                        } else { //输入数据符合要求，进行保存
                            try {
                                sql = "UPDATE teacher SET tno=?,tname=?,tsex=?,password=? WHERE tno='" + teacher.tno + "'";
                                ps = HomePage.connection.prepareStatement(sql);
                                ps.setString(1, tf[0].getText()); //SQL语句第一个?值
                                ps.setString(2, tf[1].getText()); //SQL语句第二个?值
                                ps.setString(3, tf[2].getText()); //SQL语句第三个?值
                                ...
                                rs.close();
                                ps.close();
                                teacher = new Teacher(tf[0].getText(), tf[1].getText(), tf[2].getText(), tf[3].getText());
                                for (int i = 0; i < 4; i++)
                                    tf[i].setEditable(false);
                                if (x == 1) {
                                    pn_first_2.remove(bt_save);
    ...
    public void addTeacherInfo() { //添加教师信息
        pn_first_2.removeAll();
        pn_first_2.setLayout(null);
        pn_first_2.setBounds(0, 0, 1045, 695);
        pn_first_2.setBorder(BorderFactory.createEtchedBorder());
        ...
        bt_confirm.addActionListener(new ActionListener() { //确认添加教师信息
            public void actionPerformed(ActionEvent e) {
                String sex = null;
                if (tf[0].getText().equals("") || tf[1].getText().equals("")|| tf[2].getText().equals("")||cb_sex.getSelectedIndex() == 0) {
                    lb_tips.setText("请填写完整！");
                    functionTips();
                } else { //已填写完整
                    if (cb_sex.getSelectedItem() == "          男")
                        ...
                        sex = "女";
                    teacher = new Teacher(tf[0].getText(), tf[1].getText(), sex,tf[2].getText() );
                    String sql; //SQL语句
                    PreparedStatement ps; //创建PreparedStatement类对象ps，用来执行SQL语句
                    ResultSet rs; //ResultSet类，用来存放获取的结果集
                    try {
                        ...
                        ps = HomePage.connection.prepareStatement(sql);
                        rs = ps.executeQuery(sql);
                        if (rs.next()) { //工号存在，则进行提示
                            lb_tips.setText("工号已经存在！");
                            functionTips();
                        } else { //输入数据符合要求，进行添加
                            try {
                                sql = "INSERT INTO teacher VALUES(?,?,?,?)"; //SQL语句
                                ps = HomePage.connection.prepareStatement(sql); //把操作数据库返回的结果保存到ps中
                                ps.setString(1, teacher.tno); //SQL语句第一个?值
                                ps.setString(2, teacher.tname); //SQL语句第二个?值
                                ps.setString(3, teacher.tsex); //SQL语句第三个?值
                                ps.setString(4, teacher.password); //SQL语句第四个?值
                                ps.executeUpdate(); //更新，执行插入操作
                                lb_tips.setText("教师添加成功！");
                                functionTips();
                                student = null;
                                for (int i = 0; i < 3; i++) {
                                    tf[i].setEditable(false);
    ...
    public void queryTeacherInfo_1() { //查询教师信息
        pn_second.setLayout(null);
        pn_second_1.removeAll();
        pn_second_1.setLayout(null);
        pn_second_1.setBounds(0, 0, 1045, 695);
        ...
        bt_query.addActionListener(new ActionListener() { //查询教师信息
            public void actionPerformed(ActionEvent e) {
                ...
                if (tf[0].getText().equals("") && tf[1].getText().equals("") && tf[2].getText().equals("") && tf[3].getText().equals("")) {
                    ...
                    sql = sql + " WHERE 1=1";
                    if (!tf[0].getText().equals(""))
                        sql = sql + " AND tno='" + tf[0].getText() + "'";
                    if (!tf[1].getText().equals(""))
                        sql = sql + " AND tname='" + tf[1].getText() + "'";
                    if (!tf[2].getText().equals(""))
                        sql = sql + " AND tsex='" + tf[2].getText() + "'";
                    if (!tf[3].getText().equals(""))
                        sql = sql + " AND password='" + tf[3].getText() + "'";
                }
                queryTeacherInfo_2(sql);
            }
        });
        pn_second.removeAll();
        sysAdminJFrame.repaint();
    ...
    public void queryTeacherInfo_2(String sql) { //查询教师信息结果
        String[] columnNames = {"工号", "姓名", "性别", "密码"}; //表格列名
        String[][] rowData = null; //表格数据
        int count = 0; //查询到的教师个数
        try {
            PreparedStatement ps; //创建PreparedStatement类对象ps，用来执行SQL语句
            ps = HomePage.connection.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.TYPE_FORWARD_ONLY); //把操作数据库返回的结果保存到ps中
            ResultSet rs = ps.executeQuery(sql); //ResultSet类，用来存放获取的结果集
            rs.last();
            count = rs.getRow(); //获取查询到的教师个数
            if (count == 0) { //若无符合条件的教师
                rowData = new String[1][4];
                for (int i = 0; i < 4; i++)
                    rowData[0][i] = "无";
            } else { //若有符合条件的学生
                ...
                int i = 0;
                do { //获取符合条件的学生信息
                    rowData[i][0] = rs.getString("tno"); //工号
                    rowData[i][1] = rs.getString("tname"); //姓名
                    rowData[i][2] = rs.getString("tsex"); //性别
                    rowData[i][3] = rs.getString("password") ; //密码
                    i++;
                } while (rs.next());
            }
            rs.close();
        ...
        if (count != 0) {
            MyEvent e = new MyEvent() { //点击“查看”按钮，查看信息，可修改和删除信息
                public void invoke(ActionEvent e) {
                    MyButton button = (MyButton) e.getSource();
                    visitTeacherInfo((String) table.getValueAt(button.getRow(), button.getColumn() - 3), 2, sql);
                }
            };
            table.getColumnModel().getColumn(3).setCellRenderer(new MyButtonRender("查看")); //设置表格的渲染器
            MyButtonEditor editor = new MyButtonEditor(e, "查看");
    ...
    public void allCourse() { //所有课程
        String[] columnNames = {"课程编号", "课程名", "课程学分", "教师编号", "操作"}; //表格列名
        String[][] rowData = null; //表格数据
        int count = 0; //表的元组总数
        try { //获取course表信息
            ...
            if (count == 0) { //若course表无元组
                ...
                int i = 0;
                do { //获取所有学生信息
                    rowData[i][0] = rs.getString("cno"); //课程编号
                    rowData[i][1] = rs.getString("cname"); //课程名
                    rowData[i][2] = rs.getString("score"); //课程学分
                    rowData[i][3] = rs.getString("tno"); //教师编号
                    i++;
                } while (rs.next());
            }
            rs.close();
        ...
        JPanel pn_top = new JPanel();
        pn_top.setBounds(0, 0, 1045, 50);
        JLabel lb_num = new JLabel("  共" + count + "条课程信息！  ");
        lb_num.setFont(new Font("黑体", 0, 25));
        JButton bt_add = new JButton("添加", new ImageIcon("image/add.png")), bt_export = new JButton("导出", new ImageIcon("image/导出.png"));
        bt_add.setBackground(Color.white.darker());
        bt_add.setFont(new Font("黑体", 0, 20));
        ...
        if (count != 0) {
            MyEvent e = new MyEvent() { //点击“查看”按钮，查看信息，可修改和删除信息
                public void invoke(ActionEvent e) {
                    MyButton button = (MyButton) e.getSource();
                    pn_first_2.removeAll();
                    visitCourseInfo((String) table.getValueAt(button.getRow(), button.getColumn() - 4), 1, null);
                }
            };
            table.getColumnModel().getColumn(4).setCellRenderer(new MyButtonRender("查看")); //设置表格的渲染器
            MyButtonEditor editor = new MyButtonEditor(e, "查看");
        ...
        bt_add.addActionListener(new ActionListener() { //添加课程信息
            public void actionPerformed(ActionEvent e) {
                pn_first_2.removeAll();
                addCourseInfo();
            }
    ...
    public void visitCourseInfo(String course_num, int x, String query_sql) { //查看课程信息，可修改和删除
        if (x == 1) {
            pn_first_2.removeAll();
            pn_first_2.setLayout(null);
            pn_first_2.setBounds(0, 0, 1045, 695);
        ...
        bt_back.setBorderPainted(false);
        bt_back.setCursor(new Cursor(Cursor.HAND_CURSOR));
        JButton bt_modify = new JButton("修改课程信息", new ImageIcon("image/modify.png")), bt_delete = new JButton("删除课程信息", new ImageIcon("image/delete.png")),
                bt_save = new JButton("保存", new ImageIcon("image/save.png")), bt_cancel = new JButton("取消", new ImageIcon("image/cancel.png"));
        bt_modify.setFont(new Font("黑体", 0, 17));
        bt_modify.setBounds(755, 380, 190, 40);
        bt_modify.setContentAreaFilled(false);
        ...
        try {
            String sql = "SELECT * FROM course WHERE cno='" + course_num + "'"; //SQL语句
            PreparedStatement ps; //创建PreparedStatement类对象ps，用来执行SQL语句
            ps = HomePage.connection.prepareStatement(sql); //把操作数据库返回的结果保存到ps中
            ResultSet rs = ps.executeQuery(sql); //ResultSet类，用来存放获取的结果集
            while (rs.next()) {
                course = new Course(rs.getString("cno"), rs.getString("cname"), rs.getString("score"), rs.getString("tno"));
                tf[0].setText(course.cno);
                tf[1].setText(course.cname);
                tf[2].setText(String.valueOf(course.score));
                tf[3].setText(course.tno);
            }
            rs.close();
            ps.close();
        } catch (SQLException e) {
        ...
        bt_modify.addActionListener(new ActionListener() { //修改课程信息
            public void actionPerformed(ActionEvent e) {
                for (int i = 0; i < 4; i++)
                    tf[i].setEditable(true);
                if (x == 1) {
        ...
        bt_delete.addActionListener(new ActionListener() { //删除课程信息
            public void actionPerformed(ActionEvent e) {
                lb_tips.setText("是否确定删除该课程信息？");
                if (x == 1)
                    choiceTips("");
                else
                    choiceTips(query_sql);
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    if (tf[0].getText().equals("") || tf[1].getText().equals("") || tf[2].getText().equals("") || tf[3].getText().equals("")) {
                        ...
                        boolean flag = false; //代表课程号是否重复
                        //检测学号是否重复
                        sql = "SELECT cno FROM course WHERE cno='" + tf[0].getText() + "'";
                        ps = HomePage.connection.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.TYPE_FORWARD_ONLY);
                        rs = ps.executeQuery(sql);
                        flag = rs.next();
                        rs.first();
                        flag = flag && !(tf[0].getText().equals(course.cno));
                        if (flag) { //课程号重复，进行提示
                            lb_tips.setText("课程号重复！");
                            functionTips();
                            flag = false;
                        } else { //输入数据符合要求，进行保存
                            try {
                                //sql = "UPDATE course SET cno=?,cname=?,score=?,tno=? WHERE cno='" + course.cno + "'";
                                sql = "UPDATE course SET cno=?,cname=?,score=?,tno=? WHERE cno='0001'";
                                //sql = "UPDATE course SET cno='2001',cname='高等数学a',score=20,tno='2001' WHERE cno='2001'";
                                ps = HomePage.connection.prepareStatement(sql);
                                ps.setString(1, tf[0].getText()); //SQL语句第一个?值
                                ps.setString(2, tf[1].getText()); //SQL语句第二个?值
                                ps.setInt(3, Integer.parseInt(tf[2].getText())); //SQL语句第三个?值
                                ...
                                if (rowsAffected > 0) {
                                    System.out.println("修改成功，受影响的行数：" + rowsAffected);
                                } else {
                                    System.out.println("未进行修改操作");
                                }
                                rs.close();
                                ps.close();
                                course = new Course(tf[0].getText(), tf[1].getText(), tf[2].getText(), tf[3].getText());
                                for (int i = 0; i < 4; i++)
                                    tf[i].setEditable(false);
                                if (x == 1) {
                                    pn_first_2.remove(bt_save);
    ...
    public void addCourseInfo() { //添加课程信息
        pn_first_2.removeAll();
        pn_first_2.setLayout(null);
        pn_first_2.setBounds(0, 0, 1045, 695);
        pn_first_2.setBorder(BorderFactory.createEtchedBorder());
        ...
        bt_confirm.addActionListener(new ActionListener() { //确认添加课程信息
            public void actionPerformed(ActionEvent e) {
                String sex = null;
                if (tf[0].getText().equals("") || tf[1].getText().equals("") || tf[2].getText().equals("") || tf[3].getText().equals("") ) {
                    lb_tips.setText("请填写完整！");
                    ...
                    try {
                        ...
                        ps = HomePage.connection.prepareStatement(sql);
                        rs = ps.executeQuery(sql);
                        if (rs.next()) { //课程号存在，则进行提示
                            lb_tips.setText("课程号已经存在！");
                            functionTips();
                        } else { //输入数据符合要求，进行添加
                            try {
                                sql = "INSERT INTO course VALUES(?,?,?,?)"; //SQL语句
                                ps = HomePage.connection.prepareStatement(sql); //把操作数据库返回的结果保存到ps中
                                ps.setString(1, course.cno); //SQL语句第一个?值
                                ps.setString(2, course.cname); //SQL语句第二个?值
                                ps.setInt(3, course.score); //SQL语句第三个?值
                                ps.setString(4, course.tno); //SQL语句第四个?值
                                ps.executeUpdate(); //更新，执行插入操作
                                lb_tips.setText("课程添加成功！");
                                functionTips();
                                student = null;
                                for (int i = 0; i < 4; i++) {
                                    tf[i].setEditable(false);
    ...
    public void queryCourseInfo_1() { //查询课程信息
        ...
        bt_query.addActionListener(new ActionListener() { //查询课程信息
            public void actionPerformed(ActionEvent e) {
                ...
                if (tf[0].getText().equals("") && tf[1].getText().equals("") && tf[2].getText().equals("") && tf[3].getText().equals("")) {
                    ...
                    sql = sql + " WHERE 1=1";
                    if (!tf[0].getText().equals(""))
                        sql = sql + " AND cno='" + tf[0].getText() + "'";
                    if (!tf[1].getText().equals(""))
                        sql = sql + " AND cname='" + tf[1].getText() + "'";
                    if (!tf[2].getText().equals(""))
                        sql = sql + " AND score=" + tf[2].getText();
                    if (!tf[3].getText().equals(""))
                        sql = sql + " AND tno='" + tf[3].getText() + "'";
                }
    ...
    public void queryCourseInfo_2(String sql) { //查询课程信息结果
        String[] columnNames = {"课程编号", "课程名", "课程学分", "教师编号", "操作"}; //表格列名
        String[][] rowData = null; //表格数据
        int count = 0; //查询到的课程个数
        try {
            PreparedStatement ps; //创建PreparedStatement类对象ps，用来执行SQL语句
            ps = HomePage.connection.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.TYPE_FORWARD_ONLY); //把操作数据库返回的结果保存到ps中
            ResultSet rs = ps.executeQuery(sql); //ResultSet类，用来存放获取的结果集
        ...
        if (count != 0) {
            MyEvent e = new MyEvent() { //点击“查看”按钮，查看信息，可修改和删除信息
                public void invoke(ActionEvent e) {
                    MyButton button = (MyButton) e.getSource();
                    visitCourseInfo((String) table.getValueAt(button.getRow(), button.getColumn() - 4), 2, sql);
                }
            };
            table.getColumnModel().getColumn(4).setCellRenderer(new MyButtonRender("查看")); //设置表格的渲染器
            MyButtonEditor editor = new MyButtonEditor(e, "查看");
    ...
    private void querySelectCourse() {
        ...
        bt_query.addActionListener(e -> {
            String sql = "SELECT stu_num,name,course.cno,course.cname FROM select_course,course,student"; //SQL语句
            if (!(tf[0].getText().isEmpty() && tf[1].getText().isEmpty() && tf[2].getText().isEmpty() && tf[3].getText().isEmpty())) { //有查询条件
                sql = sql + " WHERE 1=1";
                ...
                if (!tf[2].getText().isEmpty())
                    sql = sql + " AND select_course.cno='" + tf[2].getText() + "'";
                if (!tf[3].getText().isEmpty())
                    sql = sql + " AND course.cname='" + tf[3].getText() + "'";
            }
            querySelectCourse_1(sql);
        });
        pn_second.removeAll();
        sysAdminJFrame.repaint();
        pn_second.add(pn_second_1);
    ...
    private void querySelectCourse_1(String sql) {
        String[] columnNames = {"学号", "姓名", "课程号", "课程", "操作"}; //表格列名
        String[][] rowData = null; //表格数据
        int count = 0; //查询到的学生个数
        try {
            PreparedStatement ps; //创建PreparedStatement类对象ps，用来执行SQL语句
            ...
            if (count == 0) { //若无符合条件的学生
                ...
                int i = 0;
                do { //获取符合条件的学生信息
                    rowData[i][0] = rs.getString("stu_num");
                    rowData[i][1] = rs.getString("name");
                    rowData[i][2] = rs.getString("course.cno");
                    rowData[i][3] = rs.getString("course.cname") ;
                    i++;
                } while (rs.next());
            }
            rs.close();
        ...
        JPanel pn_top = new JPanel();
        pn_top.setBounds(0, 0, 1045, 50);
        JLabel lb_num = new JLabel(" 共查询到" + count + "条选课信息！ ");
        lb_num.setFont(new Font("黑体", 0, 25));
        JButton bt_back = new JButton("返回", new ImageIcon("image/返回.png"));
        bt_back.setFont(new Font("黑体", 0, 17));
        bt_back.setContentAreaFilled(false);
        ...
        if (count != 0) {
            MyEvent e = new MyEvent() { //点击“查看”按钮，查看信息，可修改和删除信息
                public void invoke(ActionEvent e) {
                    MyButton button = (MyButton) e.getSource();
                    unselect((String) table.getValueAt(button.getRow(), button.getColumn() - 2), (String) table.getValueAt(button.getRow(), button.getColumn() - 4));
                }
            };
            table.getColumnModel().getColumn(4).setCellRenderer(new MyButtonRender("退选")); //设置表格的渲染器
            MyButtonEditor editor = new MyButtonEditor(e, "退选");
            table.getColumnModel().getColumn(4).setCellEditor(editor); //设置表格的编辑器
        }
        bt_back.addMouseListener(new MouseListener() { //返回
            public void mouseEntered(MouseEvent arg0) {
    ...
    private void all_select_course() {
        String[] columnNames = {"学号", "姓名", "课程号","课程", "操作"}; //表格列名
        String[][] rowData = null; //表格数据
        int count = 0; //表的元组总数
        try { //获取student表信息
            String sql = "SELECT stu_num,name,course.cno,course.cname FROM select_course,course,student WHERE stu_num=sno && select_course.cno= course.cno"; //SQL语句
            PreparedStatement ps; //创建PreparedStatement类对象ps，用来执行SQL语句
            ps = HomePage.connection.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.TYPE_FORWARD_ONLY); //把操作数据库返回的结果保存到ps中
            ResultSet rs = ps.executeQuery(sql); //ResultSet类，用来存放获取的结果集
            rs.last();
            count = rs.getRow(); //获取选课元组总数
            if (count == 0) { //若无元组
                rowData = new String[1][5];
                for (int i = 0; i < 5; i++)
                    rowData[0][i] = "无";
        ...
        JPanel pn_top = new JPanel();
        pn_top.setBounds(0, 0, 1045, 50);
        JLabel lb_num = new JLabel("  共" + count + "条选课信息！  ");
        lb_num.setFont(new Font("黑体", 0, 25));
        JButton bt_export = new JButton("导出", new ImageIcon("image/导出.png"));
//        bt_add.setBackground(Color.white.darker());bt_add = new JButton("添加", new ImageIcon("image/add.png")),
//        bt_add.setFont(new Font("黑体", 0, 20));
//        bt_add.setCursor(new Cursor(Cursor.HAND_CURSOR));
        bt_export.setBackground(Color.green.darker());
        bt_export.setFont(new Font("黑体", 0, 20));
...
//        });
    ...
    private void add_select_course() {
    }

    private void unselect(String cno, String sno) {
        lb_tips.setText("是否选择退选？");
    ...
    private void unselect_course(String cno, String sno) {
        String sql = "DELETE FROM select_course WHERE sno=? && cno=?";
        //String sql = "INSERT INTO select_course(sno,cno,grade) VALUES"+"("+"'"+stu_num+"'"+"'"+str+"'"+""+")"; //SQL语句
        PreparedStatement ps = null;
        try {
            ps = HomePage.connection.prepareStatement(sql);
            ps.setString(1, sno);      // 设置第二个参数，对应 SQL 语句中的第二个问号
            ps.setString(2, cno);  // 设置第三个参数，对应 SQL 语句中的第三个问号，替换为实际的 grade 数据
    ...
    public void yesOperation(String str) { //选择“是”之后，根据提示内容，选择要进行的操作
        ...
        } else if (lb_tips.getText().equals("是否确定删除该教师信息？")) { //确定删除教师，进行删除操作
            try {
                String sql = "DELETE FROM teacher WHERE tno='" + teacher.tno + "'"; //SQL语句
                PreparedStatement ps; //创建PreparedStatement类对象ps，用来执行SQL语句
                ps = HomePage.connection.prepareStatement(sql); //把操作数据库返回的结果保存到ps中
                ps.executeUpdate(); //更新，执行删除操作
                lb_tips.setText("教师删除成功！");
                functionTips();
                ps.close();
                floor = null;
                if (str.equals(""))
        ...
        else if (lb_tips.getText().equals("是否确定删除该课程信息？")) { //确定删除课程，进行删除操作
            try {
                String sql = "DELETE FROM course WHERE cno='" + course.cno + "'"; //SQL语句
                PreparedStatement ps; //创建PreparedStatement类对象ps，用来执行SQL语句
                ps = HomePage.connection.prepareStatement(sql); //把操作数据库返回的结果保存到ps中
                ps.executeUpdate(); //更新，执行删除操作
                lb_tips.setText("课程删除成功！");
                functionTips();
                ps.close();
                floor = null;
                if (str.equals(""))
